...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Service role client — bypasses RLS for first-login org + user creation
    ...
    // Check if user already has a record (returning user)
    ...
    if (existingUser?.org_id) {
      console.log('[auth/callback] Returning user, redirecting to /setup')
    ...
    const redirectTo = next.startsWith('/') ? next : '/setup'
